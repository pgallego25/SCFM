...
namespace VMS.TPS
{
    ...
    public class Script
    {
        ...
        {
            UMClass UMClase = new UMClass();
          


            foreach (ExternalPlanSetup myPlan in context.Course.ExternalPlanSetups)
            {
                // float[,] Mlc=myPlanmyBeam.ControlPoints.First().LeafPositions;
                float[,] Mlc = myPlan.Beams.Where(myBeam => !(myBeam.IsSetupField)).First().ControlPoints.First().LeafPositions;
                // MessageBox.Show(Mlc[0,0].ToString());
                // string st = UMClase.DisplayPolygon(Mlc);

            }
            // string Text= UMClase.ImportTextFile("J:\\TRANSFER\\FISICA\\VarianScripting\\DatosClinacs\\600\\PDDs\\X06 OPEN 10X10 PDD.ASC");
            // MessageBox.Show(Text);
            //   string ClinacDirectory= @"J:\\TRANSFER\\FISICA\\VarianScripting\\DatosClinacs\\600\\";
            //  string PDDPath = @"J:\\TRANSFER\\FISICA\\VarianScripting\\DatosClinacs\\600\\PDDs\\";
            // string SpPath6MV600 = @"J:\\TRANSFER\\FISICA\\VarianScripting\\DatosClinacs\\600\\Sp\\SP6MV.txt";
            string TPRpath6MV600 = @"J:\\TRANSFER\\FISICA\\VarianScripting\\DatosClinacs\\600\\TPRTable.osl";
            //  string Scppath6MV600 = @"J:\\TRANSFER\FISICA\\VarianScripting\\DatosClinacs\\600\\Scp\\OUTPUT_X06.ASC";

            //    string PDDFolder = @"J:\\TRANSFER\\FISICA\\VarianScripting\\DatosClinacs\\600\\PDDs\\";
                   //      double[][] b = UMClase.ReadPDDfromFile(PDDPath);



            /*  Tuple<double[][] , double[], double[]> Resultados= UMClase.CreatePDDTable(PDDPath);
             *  
              double[][] PDDTable = Resultados.Item1;
              double[] Sizes = Resultados.Item2;
              double[] Positions = Resultados.Item3;

              UMClase.ShowMultiArray(PDDTable);
              UMClase.ShowArray(Sizes);
              UMClase.ShowArray(Positions);*/
            /*  string Text = UMClase.ImportTextFile("J:\\TRANSFER\\FISICA\\VarianScripting\\DatosClinacs\\600\\SP\\SP6MV.txt");
              MessageBox.Show(Text);
              UMClase.ReadSpTable(SPpath);
              */
            // double a = 2.5;
            // double[] ListPos=UMClase.SearchScalarInArray(a, new double[] { 1, 2, 3 });
            // MessageBox.Show(ListPos[0].ToString() + " " + ListPos[1].ToString());
            // double Resultado =UMClase.GetSPfromFieldSize(8.0, SpPath6MV600);
            // MessageBox.Show(Resultado.ToString());
            // Tuple<double[], double[], double[]> Resultado2 = UMClase.GetPDDorTPRfromFieldSize(9.0, PDDPath);
            // UMClase.ShowArray(Resultado2.Item1);
            //double[] Resultado3 = UMClase.GetPDDorTPRfromFieldSize(8.0, TPRpath6MV600);
            //MessageBox.Show(Resultado3[74].ToString());

            //  double a = UMClase.GetSPfromFieldSize(11.11111111111, SpPath6MV600);
            // MessageBox.Show(a.ToString());

            //   Tuple< double[][], double[], double[]> TprResults=UMClase.CreateTPRTableFromPDDandSP(PDDPath, SpPath6MV600);
            //  UMClase.Serialize(TprResults, ClinacDirectory +"TPRTable.osl");
            //Tuple<double[][], double[], double[]> TestArray = (Tuple<double[][], double[], double[]>)UMClase.Deserialize(ClinacDirectory + "TPRTable.osl");
            //double[][] TPRTable2 = TprResults.Item1;
            //double[] Sizes = TprResults.Item2;
            //double[] Positions = TprResults.Item3;
            //MessageBox.Show(TPRTable2[4][74].ToString());
            //MessageBox.Show(TestArray[74, 4].ToString());
            // Tuple<double[], double[], double[]> Resultado2 = UMClase.GetPDDorTPRfromFieldSize(9.0, TPRpath6MV600);
            // double[] TPR = Resultado2.Item1;
            // MessageBox.Show(TPR[74].ToString());
            // Tuple<double[][], double[]> Resultado = UMClase.ReadScpTable(Scppath6MV600);
            //double[][] ScpTable=Resultado.Item1;
            //UMClase.ShowMultiArray(ScpTable);
            //double Resultado2 = UMClase.GetScpFromFieldSize(4.564, 30.25, Scppath6MV600);
            //MessageBox.Show(Resultado2.ToString());
            double Resultado3 = UMClase.GetTPRfromFieldSizeAndDistance(10, 10, TPRpath6MV600);
            MessageBox.Show(Resultado3.ToString());
