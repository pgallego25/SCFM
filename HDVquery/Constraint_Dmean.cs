...
namespace SCFM
{
    ...
    public sealed class Constraint_Dmean : Constraint
    {
        ...
        /// </remarks>
        public Constraint_Dmean(DoseValue dose, StructureType structureType, ComparisonMode comparisonMode)
        {
            this.Dose = dose;
        ...
        /// Properties are initialized in an appropriate order to allow meaningful argument checking.
        /// </remarks>
        public Constraint_Dmean(DoseValue dose, Structure structure, StructureType structureType, ComparisonMode comparisonMode)
        {
            this.Dose = dose;
            this.VolumeUnits = null;
            this.Volume = null;
        ...
        /// <returns>The Dmean of the current OAR/PTV/other for the current treatment plan (to compare against <c>this</c> constraint). Dmean has the same representation (absolute/relative) and units than <c>this</c> constraint. Type: <see cref="System.Double"/>.</returns>
        /// <remarks>
        ...
        protected override double getCurrentValue(PlanningItem myPlan, double cumulativePrescriptionDose = 0.0)
        {
            if (myPlan is PlanSum && this.Dose.Unit == DoseValue.DoseUnit.Percent)
                return myPlan.GetDVHCumulativeData(this.Structure,
                    ...
                    0.001).MeanDose.Dose * 100.0 / cumulativePrescriptionDose;
            else
                return myPlan.GetDVHCumulativeData(this.Structure,
