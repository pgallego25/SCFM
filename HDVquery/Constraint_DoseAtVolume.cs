...
namespace SCFM
{
    ...
    public sealed class Constraint_DoseAtVolume : Constraint
    {
        ...
        /// <value><see cref="System.Nullable"/> of type <see cref="System.Double"/> <c>base.Volume</c>.</value>
        /// <remarks>Read-only property because index is determined by the type: <see cref="Volume"/> for this type.</remarks>
        ...
        /// <remarks>Read-only property because index units are determined by the type: either "cc" or "%" depending upon the <see cref="VolumeUnits"/> property.</remarks>
        ...
        public Constraint_DoseAtVolume(DoseValue dose, double volume, VolumePresentation volumeUnits, StructureType structureType, ComparisonMode comparisonMode)
        {
            this.Dose = dose;
        ...
        /// <param name="comparisonMode">The comparison mode between actual value and the constraint value. Type: <see cref="ComparisonMode"/>.</param>
        /// <remarks>Properties are initialized in an appropriate order to allow meaningful argument checking.</remarks>
        public Constraint_DoseAtVolume(DoseValue dose, double volume, VolumePresentation volumeUnits, Structure structure, StructureType structureType, ComparisonMode comparisonMode)
        {
            this.Dose = dose;
            this.VolumeUnits = volumeUnits;
            this.Volume = volume;
        ...
        /// <returns>The D_V of the current OAR/PTV/other for the current treatment plan (to compare against <c>this</c> constraint). D_V has the same representation (absolute/relative) and units than <c>this</c> constraint, and it is evaluated at the same point (<see cref="Index"/>). Type: <see cref="System.Double"/>.</returns>
        /// <remarks>
        ...
        protected override double getCurrentValue(PlanningItem myPlan, double cumulativePrescriptionDose = 0.0)
        {
            if (myPlan is PlanSum && this.Dose.Unit == DoseValue.DoseUnit.Percent)
                return myPlan.GetDoseAtVolume(this.Structure,
                    (double)this.Volume,
                    (VolumePresentation)this.VolumeUnits,
                    DoseValuePresentation.Absolute).Dose * 100.0 / cumulativePrescriptionDose;
            else
                return myPlan.GetDoseAtVolume(this.Structure,
                    (double)this.Volume,
                    (VolumePresentation)this.VolumeUnits,
                    this.Dose.Unit == DoseValue.DoseUnit.Percent ? DoseValuePresentation.Relative : DoseValuePresentation.Absolute).Dose;
        }
